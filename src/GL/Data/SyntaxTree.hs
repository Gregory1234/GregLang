...
data AST = AST [GLImport] GLClass

data GLImport = GLImport [String]

data GLClass = GLClass String [GLFun]

data GLFun = GLFun String [String] [GLStat]
...
instance Treeable GLFun where
  toTree (GLFun n as s) = let (Node x y) = listToTree ("fun " ++ n) s in Node x (listToTree "args" as:y)

instance Treeable GLStat where
  toTree (SIf e s) = Node "if" [toTree e,Node "then" [toTree s]]
  toTree (SIfElse e s1 s2) = Node "if" [toTree e,Node "then" [toTree s1],Node "else" [toTree s2]]
  toTree (SFor s1 e s2 s3) = Node "for" [toTree s1,toTree e,toTree s2,Node "do" [toTree s3]]
